...
class Ercf:
        ...
        for step in range( int( homing_length / abs(step_length) ) + 1 ):
            if bool(sensor.runout_helper.filament_present) == sensor_state:
                if step * abs(step_length) >= homing_length :
                    self.gcode.respond_info("Sensor: "+ str(sensor.runout_helper.filament_present))
                    self.gcode.respond_info(str(step) + " Steps, " + str(step_length) + "Step Length, " + str(homing_length) + "Homing Length")
                    if not bool(sensor.runout_helper.filament_present):
                         self.gcode.respond_info("Sleeping 5sec to re-check sensor...") 
                         time.sleep(5)
                    if not bool(sensor.runout_helper.filament_present):
    ...
    def cmd_ERCF_FINALIZE_LOAD(self, gcmd):
        ...
        if final_encoder_pos < 10. :
            self.gcode.respond_info(
                "Filament seems blocked between the extruder and the nozzle (%s) ,"
                " calling %s..."
                %(str(final_encoder_pos), self.MACRO_PAUSE))
            self.gcode.run_script_from_command(self.MACRO_PAUSE)
            return
